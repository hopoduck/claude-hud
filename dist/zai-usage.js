...
export function detectZaiPlatform(env = process.env) {
...
function readCache(now) {
    try {
        ...
        if (entry.data) {
            const d = entry.data;
            if (typeof d.fiveHourResetAt === 'string')
                d.fiveHourResetAt = new Date(d.fiveHourResetAt);
            if (typeof d.sevenDayResetAt === 'string')
                d.sevenDayResetAt = new Date(d.sevenDayResetAt);
        }
...
function mapResponse(response) {
    const tokensLimit = response.limits?.find((l) => l.type === 'TOKENS_LIMIT');
    const fiveHour = tokensLimit
        ? Math.min(100, Math.max(0, Math.round(tokensLimit.percentage)))
        : null;
    const fiveHourResetAt = tokensLimit?.nextResetTime != null
...
function fetchApi(baseUrl, token) {
...
export async function getZaiUsage(env = process.env) {
...
//# sourceMappingURL=zai-usage.js.map